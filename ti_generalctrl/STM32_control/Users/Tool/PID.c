...
#include "arm_math.h"
// 引用陀螺仪的数据
extern struct GyroData GyroData;   // 角速度
extern struct AngleData AngleData; // 角度
...
// pid_init(&Angle, 1, 0, 1, 1, 1000, 1000, 1, 1000, 1100, 1000, 0, 0, PID_D_First_DISABLE, PID_I_Separate_DISABLE, PID_I_Variable_Speed_DISABLE);
...
float pid_calculate(PIDController *pid, float current_value, float target)
{
    ...
    // 积分限幅
    pid->Intergral = limit(pid->Intergral, -pid->I_Out_MAX / pid->Ki, pid->I_Out_MAX / pid->Ki);
    float I_Out = pid->Ki * pid->Intergral;

    // 积分分离
    if (pid->I_Separate_Mode == PID_I_Separate_ENABLE && abs_error > pid->I_Separate_Threshold)
    {
    ...
    {
        D_Out = pid->Kd * (target - pid->pre_target) / pid->D_T; // Derivative of the error
    }
    else
    {
        D_Out = pid->Kd * (error - pid->pre_error) / pid->D_T; // Derivative of the current value
    }
...
float pid_calculate_with_error(PIDController *pid, float error)
{
    ...
    if (pid->D_First_Mode == PID_D_First_ENABLE)
    {
        D_Out = pid->Kd * (pid->target - pid->pre_target) / pid->D_T; // Derivative of the target
    }
    else
    {
        D_Out = pid->Kd * (error - pid->pre_error) / pid->D_T; // Derivative of the error
    }
...
float Angle_PIDcaculate(PIDController *pid, float angledata, float gyrodata, float target)
{
    ...
    // 计算PID输出
    float output = pid->Kp * error + pid->Ki * pid->Intergral + pid->Kd * gyrodata;
...
float PID_Calculate_DSP(PIDController *pid, float current_value, float target)
{
    ...
    arm_pid_init_f32(&pid_instance, 1); // Initialize the PID instance

    // Set PID parameters
    pid_instance.Kp = pid->Kp;
    pid_instance.Ki = pid->Ki;
    pid_instance.Kd = pid->Kd;

    // Calculate the PID output
    float output = arm_pid_f32(&pid_instance, target - current_value);

    // Apply feedforward gain if needed
    output += pid->kf * (target - pid->pre_target);
    ...
    pid->pre_target = target;

    return output;
}

// 使用DSP库的PID计算增量式
float PID_Calculate_DSP_Witherror(PIDController *pid, float error)
{
    // Create a PID instance
    arm_pid_instance_f32 pid_instance;
    ...
    // Calculate the PID output
    volatile float output = arm_pid_f32(&pid_instance, error);
